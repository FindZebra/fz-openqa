...
class MultiFaissHandler(IndexHandler):
    ...
    def _build(
        ...
        **kwargs,
        ...
        if doc_ids is None:
            raise ValueError("Document ids must be provided via the argument `doc_ids`.")
    ...
    def get_indexes_map(self, doc_ids: List[int]) -> Dict[int, str]:
        return {int(doc_id): str(self.path / f"{doc_id}-doc-index") for doc_id in set(doc_ids)}
    ...
    def __call__(
        ...
        """Call the index."""
        udoc_ids = set(doc_ids) if isinstance(doc_ids, list) else set(doc_ids.unique().tolist())
        if doc_ids is None:
