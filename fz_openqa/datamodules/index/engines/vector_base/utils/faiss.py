...
def compute_centroids(
    ...
    *,
    ...
    index = faiss.index_cpu_to_gpu_multiple(vres, vdev, faiss.IndexFlat(d, faiss_metric))

    # train the index
    logger.info(
        f"Train clustering index with {n_centroids} centroids " f"with max. {n_train} vectors.."
    )
    t0 = time.time()
