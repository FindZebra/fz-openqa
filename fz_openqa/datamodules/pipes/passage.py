...
class GeneratePassages(Pipe):
    ...
    required_keys = ["document.input_ids", "document.attention_mask", "document.offset_mapping"]
    ...
    def _call_batch(self, batch: Batch, **kwargs) -> Batch:
        ...
        indexes, output = self.generate_passages_for_all_keys(
            batch,
            keys=["document.input_ids", "document.attention_mask", "document.offset_mapping"],
            args=self.args,
        ...
        for idx, example in enumerate(examples[first_key]):
            if append_document_titles:
                ...
                args = get_title_tokens(
                    args=args, arg_key=first_key, temp_input=examples[f"title.{input_key}"][idx]
                )
