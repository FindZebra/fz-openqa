...
if __name__ == "__main__":
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        handler = FaissHandler(
            path=path, index_factory="IVF100k,PQ32x8", nprobe=32, faiss_train_size=1_000_000
        )

        # Create a dataset
        vectors = torch.randn(10_000_000, 32)
        ...
        rich.print(f"Scores: {scores[:3]}")
        rich.print(f"Indices: {indices[:3]}")
