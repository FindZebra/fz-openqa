...
class TestFaissIndex(TestIndex):
    ...
    def _init_index(self) -> Index:
        return self._init_index_with(corpus=self.corpus, model=self.model, collate=self.corpus_collate)
...
class TestFaissIndexWithTrainer(TestFaissIndex):
    ...
    def _init_index_with(**kwargs) -> Index:
        callbacks = [
            StorePredictionsCallback(name="document" ),
            StorePredictionsCallback(name="question")
        ]
        trainer = Trainer(callbacks=callbacks, checkpoint_callback=False, logger=False)
