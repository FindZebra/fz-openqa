...
b0 = {'question.text': "What is the symptoms of post polio syndrome?",
      "answer.target": 0, "answer.text": ["Post polio syndrome"], 'answer.synonyms': [],
      "document.text": [
...
b2 = {
    ...
    # noqa: E501
    'answer.target': 1,
    'answer.text': ["Acute cholecystitis", "Gallbladder cancer", "Choledocholithiasis",
                    "Pancreatitis"], 'answer.cui': ['C0235782'],
    "answer.synonyms": ["Gallbladder Carcinoma", "Malignant neoplasm of gallbladder"],
    "document.text": [
...
b3 = {
    ...
    # noqa: E501
    'answer.target': 0,
    'answer.text': ["Psoriatic arthritis", "Arthritis mutilans", "Rheumatoid arthritis",
                    "Mixed connective tissue disease"], 'answer.cui': ['C0003872'],
...
b4 = {
    ...
    'answer.synonyms': ["error", "failed", "attending", "Attending (action)", "attends", "failing",
                        "To", "Attending (provider role)", "attended", "Tryptophanase", "attend",
                        "fail", "fails", "Togo"],
    "document.text": [
...
b5 = {
    ...
    'answer.synonyms': ['Molecular Chaperones', 'Future', 'Patient Visit', 'Usage',
                        'Use - dosing instruction imperative', 'Visit', 'Visit Name',
                        'utilization qualifier', 'Does ask questions'],
    "document.text": [
        "is. Similarly, domestic travel may have exposed patients to pathogens that are not normally found in their local environment and therefore may not routinely be considered in the differential diagnosis. For example, a patient who has recently visited California or Martha’s Vineyard may have been exposed to Coccidioides immitis or Francisella tularensis, respectively. Beyond simply identifying locations that a patient may have visited, the physician needs to delve deeper to learn what kinds of activities and behaviors the patient engaged in during travel (e.g., the types of food and sources of water consumed, freshwater swimming, animal exposures) and whether the patient had the necessary immunizations and/or took the necessary prophylactic medications prior to travel; these additional exposures, which the patient may not think to report without specific prompting, are as important as exposures during a patient’s routine daily living.Host-Specific Factors Because many opportunist",
        # noqa: E501
        "", ""]}
...
b6 = {
    ...
    # noqa: E501
    'answer.target': 0, 'answer.text': ['Obtain a urine analysis and urine culture'],
    'answer.synonyms': ['Urinalysis', 'Acquisition (action)', 'Obtain', 'Urine for culture'],
    "document.text": [
...
b7 = {
    ...
    # noqa: E501
    'answer.target': 0, 'answer.text': ['Ketotifen eye drops'],
...
class TestRelevanceClassifier(TestCase):
    ...
    def test_exact_match(self):
        ...
        # (b2) {answer.text : "Gallbladder cancer" }. The "Gallbladder cancer" is not written in the document, though, the document is clearly about "Gallbladder cancer" so should find 0 matches
        self.assertEqual(output['document.match_score'][2][0], 0)
        # (b3) {answer.text : "Psoriatic arthritis" }. Should succeed on 3 matches, because ExactMatch matches the answer.text to the document
        self.assertEqual(output['document.match_score'][3][0], 3)
        # (b4) {answer.text : "Tell the attending that he cannot fail to disclose this mistake" }. The answer.text is too difficult to match to any passage of the corpus and extract meaning from so should find 0 matches
        self.assertEqual(output['document.match_score'][4][0], 0)
        # (b5) {answer.text : "Ask closed-ended questions and use a chaperone for future visits" }. The answer.text is too difficult to match to any passage of the corpus and extract meaning from so should find 0 matches
        self.assertEqual(output['document.match_score'][5][0], 0)
        # (b6) {answer.text : "Obtain a urine analysis and urine culture" }. The answer.text is too difficult to match to any passage of the corpus and extract meaning from so should find 0 matches
        self.assertEqual(output['document.match_score'][6][0], 0)
        # (b7) {answer.text : "Ketotifen eye drops" }. The document has nothing to do with "Ketofin", though "eye drops" is mentioned once or twice so should find 0 matches
    ...
    def test_scispacy_match(self):
        classifier = ScispaCyMatch(interpretable=True, model_name="en_core_sci_sm",
                                   linker_name="umls", filter_tui=True)
        classifier = self._wrap_classifier(classifier)
        output = classifier(copy(self.batch))
        ...
        self.assertEqual(output['document.match_score'][1][0], 0)
        # (b2) {answer.text : "Gallbladder cancer" }. The extract of aliases succeed to find 4 matches, e.g. the match "Carcinoma of the gallbladder" to the document
        self.assertEqual(output['document.match_score'][2][0], 1)
        # (b3) {answer.text : "Psoriatic arthritis" }. ExactMatch matches the answer.text to the document so should succeed on 4 matches
        self.assertEqual(output['document.match_score'][3][0], 3)
        # (b4) {answer.text : "Tell the attending that he cannot fail to disclose this mistake" }. The answer.text is too difficult to match to any passage of the corpus and extract meaning from so should find 0 matches
        self.assertEqual(output['document.match_score'][4][0], 0)
        # (b5) {answer.text : "Ask closed-ended questions and use a chaperone for future visits" }. The answer.text is too difficult to match to any passage of the corpus and extract meaning from so should find 0 matches
        self.assertEqual(output['document.match_score'][5][0], 2)
        # (b6) {answer.text : "Obtain a urine analysis and urine culture" }. It's not possible to match the extracted meaning (aliases) to the document, so should succeed on 0 matches
        self.assertEqual(output['document.match_score'][6][0], 0)
        # (b7) {answer.text : "Ketotifen eye drops" }. The document has nothing to do with "Ketofin", though "eye drops" is mentioned once or twice so should find 0 matches
        self.assertEqual(output['document.match_score'][7][0], 0)
