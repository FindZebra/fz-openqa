...
The main computation should be implemented in the `_step()` and `_step_end()` methods of the `PLModule`.
...
![Lightning PLModule data flow](.assets/lighning_steps.png)
...
During evaluation, data from each component is managed separately (so each PLModule send and receive its own data between `_step` and `_step_end`).
